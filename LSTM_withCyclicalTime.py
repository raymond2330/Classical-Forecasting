...
class LSTM_Forecaster(nn.Module):
    def __init__(self, n_features, window, horizon,
        ...
        self.enc3 = nn.LSTM(input_size=enc2,       hidden_size=enc3, batch_first=True)
        self.enc_ln = nn.LayerNorm(enc3)
    ...
    def forward(self, x):
        ...
        h3, _ = self.enc3(h2)              # [B,W,enc3]
        h3 = self.enc_ln(h3)

        # Attention pooling across time
        a = self.attn(h3)                  # [B,W,1]
        a = torch.softmax(a, dim=1)
        ctx = (a * h3).sum(dim=1)          # [B,enc2]
...
def main():
    ...
    # Load
    df = pd.read_csv('tmp/alert_collapsed.csv', parse_dates=['DateTime'], index_col='DateTime').fillna(0)
    ...
    # We already added cyclical columns directly to df: 'hod_sin','hod_cos','dom_sin','dom_cos'
    cal_cols = ['hod_sin', 'hod_cos', 'dow_sin', 'dow_cos', 'doy_sin', 'doy_cos']
    # protect in case some are missing
    cal = df[ [c for c in cal_cols if c in df.columns] ].copy()

    exo_cols: List[str] = []
    if 'Temperature' in df.columns: exo_cols.append('Temperature')
    if 'Outage' in df.columns:      exo_cols.append('Outage')

    # Add policy binaries as exogenous (keep holidays / alert flags). Do NOT append weekday one-hot columns:
    policy_cols = ['Holiday', 'ECQ', 'GCQ', 'ALERT_LEVEL']

    for col in policy_cols:
        if col in df.columns:
            exo_cols.append(col)
    
    # drop one-hot weekday columns if present (we have cyclical time features instead)
    weekday_onehots = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']
    existing_weekdays = [c for c in weekday_onehots if c in df.columns]
    if existing_weekdays:
        df.drop(columns=existing_weekdays, inplace=True)
    ...
    df.head(10).to_csv("tmp/processed_features.csv")
    ...
    model = LSTM_Forecaster(n_features=n_features, window=W, horizon=H,
        enc1=256, enc2=128, enc3=64, latent_dim=64, p_drop=0.2
        ).to(device)
