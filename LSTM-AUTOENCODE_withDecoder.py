...
class LSTMAE_Forecaster(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reconstruction (decode latent back to input window)
        z_seq = z.unsqueeze(1).repeat(1, W, 1)   # [B,W,latent]
        d1, _ = self.dec1(z_seq)
        d2, _ = self.dec2(d1)
        dflat = d2.reshape(B, -1)
        xhat = self.dec_proj(dflat).reshape(B, W, F)  # [B,W,F]
...
def main():
    ...
    # Load
    df = pd.read_csv('Demand_with_Temperature_noOutage.csv', parse_dates=['DateTime'], index_col='DateTime').fillna(0)
    ...
    for ep in range(1, total_epochs + 1):
        ...
        for Xb, Yseqb in train_loader:
            ...
            yhat, xhat = model(Xb)
            forecast_loss = f_fn(yhat, Yseqb)
            recon_loss = torch.nn.functional.mse_loss(xhat, Xb)
            loss = forecast_loss + 0.1 * recon_loss   # λ=0.1 for reconstruction
